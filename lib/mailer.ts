...
export async function sendConfirmEmail(to: string, name: string, token: string) {
  const link = `${process.env.NEXT_PUBLIC_SITE_URL}/api/auth/confirm?token=${encodeURIComponent(token)}`
  ...
  await resend.emails.send({
    ...
    html: `
      <div style="font-family:sans-serif;max-width:480px;margin:0 auto;padding:32px;background:#f8fafc;border-radius:16px">
        <h2 style="color:#1d4ed8;margin-bottom:8px">Саламатсызбы, ${esc(name)}!</h2>
...
export async function sendResetEmail(to: string, name: string, token: string) {
  const link = `${process.env.NEXT_PUBLIC_SITE_URL}/reset-password?token=${encodeURIComponent(token)}`
  const resend = getResend()
  if (!resend) {
  ...
  await resend.emails.send({
    ...
    html: `
      <div style="font-family:sans-serif;max-width:480px;margin:0 auto;padding:32px;background:#f8fafc;border-radius:16px">
        <h2 style="color:#dc2626;margin-bottom:8px">Саламатсызбы, ${esc(name)}!</h2>
